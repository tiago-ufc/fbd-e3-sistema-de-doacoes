...
def grafico_instituicoes(cnx):
    """Gráfico de barras mostrando instituições e suas campanhas"""
    try:
        df = _safe_df_query(
        ...
        if df.empty:
            return pn.pane.Alert('Nenhuma instituição cadastrada.', alert_type='warning')
        ...
        # Define ticks inteiros no eixo X
...
def grafico_doacoes_por_campanha(cnx):
    ...
    try:
        df = _safe_df_query(
            "SELECT c.nome, COUNT(d.id_doacao)::INTEGER as total FROM doacao d JOIN campanha c ON d.id_campanha = c.id_campanha GROUP BY c.nome ORDER BY total DESC",
            cnx,
        )
        if df.empty:
        ...
        for i, v in enumerate(df["total"]):
            ax.text(i, v + 0.2, str(int(v)), ha='center', va='bottom', fontweight='bold')
...
def grafico_doacoes_por_doador(cnx):
    ...
    try:
        df = _safe_df_query(
            "SELECT u.nome, COUNT(d.id_doacao)::INTEGER as total FROM doacao d JOIN doador doa ON d.cpf_cnpj_d = doa.cpf_cnpj_d JOIN usuario u ON doa.cpf_cnpj_d = u.cpf_cnpj GROUP BY u.nome ORDER BY total DESC LIMIT 10",
            cnx,
        )
        if df.empty:
        ...
        # Define ticks inteiros no eixo X
        max_total = df["total"].max()
        ax.set_xticks(range(0, int(max_total) + 2))
...
def grafico_campanhas_por_status(cnx):
    """Gráfico de barras mostrando campanhas por status"""
    try:
        df = _safe_df_query(
            "SELECT status, COUNT(id_campanha) as total FROM campanha GROUP BY status",
            cnx,
        ...
        fig, ax = plt.subplots(figsize=(8, 6))
        colors = ['#2ca02c', '#ff7f0e', '#d62728']
        ax.bar(df["status"], df["total"], color=colors[:len(df)], edgecolor='black')
        ax.set_title("Campanhas por Status", fontsize=14, fontweight='bold')
